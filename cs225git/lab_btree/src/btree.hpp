...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if(first_larger_idx >= 0 && first_larger_idx < subroot->elements.size() 
...
// template <class K, class V>
// bool BTree<K, V>::isLeaf_(const BTreeNode* root) const {
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    // insert pointer to point to new right node
    parent->children.insert(child_itr, new_right); 

    //find location of where to insert median into parent
    size_t location = insertion_idx(parent->elements, child->elements[mid_elem_idx]);

    //insert it there
    parent->elements.insert(parent->elements.begin() + location, child->elements[mid_elem_idx]);

    //create subvectors on left and right of median to attach to children of parent
    std::vector<DataPair> rightMidEls(mid_elem_itr + 1, child->elements.end());
    std::vector<DataPair> leftMidEls(child->elements.begin(), mid_elem_itr);
    std::vector<BTreeNode*> rightMidChilds(mid_child_itr, child->children.end());
    std::vector<BTreeNode*> leftMidChilds(child->children.begin(), mid_child_itr);

    //assign the subvectors
    new_left->elements = leftMidEls;
    new_left->children = leftMidChilds;
    new_right->elements = rightMidEls;
    new_right->children = rightMidChilds;
}
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    for(auto it = subroot->elements.begin(); it != subroot->elements.end(); it++) {
        if((*it).key == pair.key) {
            exists = true;
            break;
        }
    ...
    /* TODO Your code goes here! */
    if(subroot->is_leaf && !exists) {
        subroot->elements.insert(subroot->elements.begin() + first_larger_idx, pair);
    } else {
        size_t child_index = first_larger_idx;
        
        if(exists) {
            return;
        }
        insert(subroot->children[child_index], pair);  
        
        if(subroot->children[child_index]->elements.size() >= order) {
            split_child(subroot, child_index);
