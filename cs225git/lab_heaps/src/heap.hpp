...
void heap<T, Compare>::heapifyDown(size_t currentIdx)
{
    ...
    size_t max = currentIdx;

    if (left < _elems.size() && higherPriority(_elems[left], _elems[max]))
        max = left;
 
    if (right < _elems.size() && higherPriority(_elems[right], _elems[max]))
        max = right;
...
heap<T, Compare>::heap(const std::vector<T>& elems) 
{
    ...
    for(int i = int(elems.size())/2 - 1; i >= 0; i--) {
        heapifyDown(i);
    }
