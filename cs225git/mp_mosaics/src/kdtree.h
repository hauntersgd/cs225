...
class KDTree
{
  ...
  public:
    ...
    void insert(const vector<Point<Dim>>& newPoints, int splitDim, KDTreeNode*& cur);

    KDTreeNode* neighborHelper(const Point<Dim>& query, int dim, KDTreeNode* node) const;

    double squaredDistance(const Point<Dim>& point1, const Point<Dim>& point2) const;
