...
Animation FloodFilledImage::animate(unsigned frameInterval) const {
  ...
  Animation animation;
  PNG png = *png_;
  animation.addFrame(png); // initial frame
  unsigned counter = 0;

  for(unsigned i = 0; i < trav.size(); i++) {
    Traversals::ImageTraversal a = *(trav[i]);
    for(auto it = a.begin(); it != a.end(); ++it) {
      if(counter == frameInterval) {
        animation.addFrame(png);
        counter = 0;
      }
      HSLAPixel &curPix = png.getPixel((*it).x, (*it).y); // x = 40, y = 40
      curPix = (*(color[i])).getColor((*it).x, (*it).y);
      counter++;

    }
  }

  animation.addFrame(png); // final frame
